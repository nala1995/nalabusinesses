...
const StripeCheckoutButton = ({ stripeProductId }) => {
  const handleStripeCheckout = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/create-checkout-session', { stripeProductId });
...
/*   const StripeCheckoutButton = ({ stripeProductId }) => {
    const handleStripeCheckout = async () => {
      try {
        ...
        const stripe = await loadStripe('pk_test_51OwtJdBL3mxlGG1njJji5GY9nkCF3jb4F2gHrEBRIQHAYG0OgLRrGj7ZQPntNPvHduvkHrEHoaULn4MlKYiP6hMh00mUOthkCW');
  ...
  return (
    <button className='stripe-btn' type="button" onClick={handleStripeCheckout}>
